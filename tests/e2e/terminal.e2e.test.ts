...
describe('terminal MCP end-to-end', () => {
  it('runs a persistent signed local PTY through the actual MCP v2 client', async () => {
    ...
    expect(surfaceUi?.resourceUri).toBe('ui://terminal/v11.html');
    expect(surfaceMeta?.['openai/outputTemplate']).toBe('ui://terminal/v11.html');
    ...
    const resourceResult = await client.readResource({ uri: 'ui://terminal/v11.html' });
