...
describe('ChatGPT widget origin policy', () => {
  it('allows the canonical ChatGPT sandbox to load terminal UI assets', async () => {
    ...
    const response = await fetch(`${base}/terminal-ui/styles.css`, { headers: { Origin: chatGptOrigin } });
  ...
  it('allows legitimate subdomains in the ChatGPT widget sandbox family', async () => {
    ...
    const response = await fetch(`${base}/terminal-ui/styles.css`, { headers: { Origin: origin } });
    expect(response.status).toBe(200);
    expect(response.headers.get('access-control-allow-origin')).toBe(origin);
  });

  it('lets the ChatGPT sandbox reach terminal SSE instead of the MCP origin guard', async () => {
    const base = await startRuntime();
  ...
  it('allows the ChatGPT sandbox to reach the UI reload SSE endpoint', async () => {
    const base = await startRuntime();
    const controller = new AbortController();
    const response = await fetch(`${base}/terminal-ui/reload`, {
      headers: { Origin: chatGptOrigin }, signal: controller.signal,
    });
    expect(response.status).toBe(200);
  ...
  it('keeps /mcp strict even for the trusted widget sandbox', async () => {
    const base = await startRuntime();
    const response = await fetch(`${base}/mcp`, { headers: { Origin: chatGptOrigin } });
    expect(response.status).toBe(403);
    expect((await response.json()).error?.message).toContain('Invalid Origin');
  });

  it('rejects lookalike and unrelated browser origins', async () => {
    const base = await startRuntime();
    for (const origin of ['https://web-sandbox.oaiusercontent.com.evil.example', 'https://evil.example', 'http://web-sandbox.oaiusercontent.com']) {
      const response = await fetch(`${base}/terminal-ui/styles.css`, { headers: { Origin: origin } });
      expect(response.status).toBe(403);
