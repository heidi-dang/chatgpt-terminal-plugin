...
describe('gateway replay integrity', () => {
  ...
  it('releases permanently disconnected agent and session records after the retention window', async () => {
    const root = await mkdtemp(join(tmpdir(), 'terminal-gateway-disconnected-retention-'));
    cleanup.push(() => rm(root, { recursive: true, force: true }));
    ...
    if (!address || typeof address === 'string') throw new Error('Unable to allocate gateway disconnected-retention test port.');
    ...
    socket.send(JSON.stringify({
      ...
      agent: {
        ...
        hostname: 'gateway-disconnected-retention-test',
        display_name: 'Gateway disconnected retention test',
        platform: 'linux',
        architecture: 'x64',
        online: true,
    ...
    expect(await nextMessage(socket)).toEqual({
      ...
      sequences: { 'session-disconnected': 0 },
    });
    expect(gateway.listAgents('owner-a')).toHaveLength(1);
    expect(gateway.listSessions('owner-a')).toHaveLength(1);

    const closed = waitForClose(socket);
    socket.close();
    await closed;
    await waitUntil(() => gateway.listSessions('owner-a')[0]?.status === 'disconnected', 1000);

    await waitUntil(() => gateway.listAgents('owner-a').length === 0 && gateway.listSessions('owner-a').length === 0, 2000);
  });
