...
4. Call `terminal_semantic_close` when the semantic workspace is no longer needed.
...
The fixed semantic surface is read-only with respect to workspace contents and is therefore available to `read-only` identities. It cannot choose arbitrary LSP methods. Raw `terminal_lsp_*` operations retain the existing stricter process/mutation authorization path.
