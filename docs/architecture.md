...
The UI is a single-file React/xterm.js watch-only MCP App resource at `ui://terminal/v3.html`. It consumes the initial `terminal_start` result and renders real PTY activity from the ordered terminal SSE stream. Stdout/stderr chunks are validated immediately by sequence but coalesced into at most one xterm write per animation frame to minimize paint overhead. The widget does not accept keyboard input and exposes no terminal-control toolbar; ChatGPT remains the actor that writes, interrupts and closes sessions. The Apps bridge is used only for debounced PTY resize and app-only stream capability refresh/status recovery. A separate UI-reload SSE channel lets HTML/CSS/JS-only v3 builds self-reload in the mounted iframe while preserving session, stream capability and terminal cursor.
