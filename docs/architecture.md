...
The UI is a single-file, static-first vanilla MCP App resource at `ui://terminal/v13.html`. Its HTML contains a visible watch-only terminal shell before JavaScript executes, so a host/WebView runtime failure cannot degrade into an empty card. The runtime prefers the standard MCP Apps JSON-RPC bridge and can bootstrap through ChatGPT's `window.openai` compatibility bridge on native hosts that do not hydrate the parent-window handshake. It progressively attaches the turn surface, current PTY and short-lived ordered terminal SSE capability. Direct SSE is the preferred transport; if the host WebView cannot establish `EventSource`, or a refreshed tool result omits the SSE capability, the widget falls back to bounded `terminal_read` long-polling while continuing background SSE recovery. Stdout/stderr chunks from either transport are sequence-validated immediately and coalesced into at most one DOM append per animation frame. The widget accepts no keyboard input and exposes no terminal-control toolbar; ChatGPT remains the actor that writes, interrupts and closes sessions. The bridge handles app-only stream refresh/status recovery, host context, graceful teardown, and size notifications. A separate `/terminal-ui/reload` SSE channel performs CSS-only hot reload; the application document is never replaced.
...
The widget tracks the most recent accepted event sequence and accepts only the next contiguous sequence. Duplicate/stale events are ignored. A forward gap, malformed frame, or EventSource failure closes the old source and starts bounded `terminal_read` fallback immediately while obtaining a fresh capability through `terminal_stream_refresh(session_id, after)` via the MCP Apps bridge. The client does not proactively tear down a healthy SSE just because its capability is approaching expiry; the server enforces the expiry and ends that response. The resulting EventSource error therefore has a deterministic recovery path that keeps output flowing over MCP reads until the replacement SSE opens. Native EventSource retry is never allowed to reuse an expired capability URL. If the requested cursor has already fallen behind retained history, the widget obtains `terminal_status`, advances to the authoritative server cursor, visibly marks the lost-output gap, and resumes from there.
