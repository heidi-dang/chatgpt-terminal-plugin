...
The browser terminal stream is not the MCP endpoint. ChatGPT connects to `/mcp`; the watch-only MCP App receives a separate short-lived, read-only SSE capability through tool-result metadata. The current bootstrap resource is `ui://terminal/v3.html`. The mounted v3 widget can hot-reload subsequent HTML/CSS/JS-only UI builds through `/terminal-ui/reload` + `/terminal-ui/runtime.html` while preserving the active session and terminal cursor, so those visual tweaks do not require a plugin refresh. Bump the resource URI again when the MCP/UI contract changes in a way that requires hosts to discard the v3 bootstrap contract.
