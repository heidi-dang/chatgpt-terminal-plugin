...
Set `MCP_DEFAULT_EXECUTION_PROFILE` to the least privilege appropriate for tokens that do not carry an explicit profile claim. `TERMINAL_CLOSED_SESSION_RETENTION_MS` controls how long final session metadata/events remain available for status, UI reconnect, and post-mortem reads before agent/server memory is released. The same window bounds server-side records for agents and active sessions that stay disconnected permanently; reconnecting agents can re-register and resume from their retained local snapshot. Use `deploy/server-environment.example` as the complete non-secret template; the runtime rejects malformed public URLs, insecure production OAuth endpoints, empty required scopes, and route collisions at startup.
...
- An external full-agent smoke credential is optional. For origin-managed JWT OAuth, set `TERMINAL_SMOKE_BEARER_TOKEN` to a dedicated bearer accepted by the public MCP endpoint.
- For Cloudflare Access, configure both `TERMINAL_SMOKE_CF_ACCESS_CLIENT_ID` and `TERMINAL_SMOKE_CF_ACCESS_CLIENT_SECRET` for a narrowly scoped service identity; a partial pair is rejected. When an external smoke principal is configured, it must own at least one online non-read-only terminal agent so the deployment can verify a real PTY and SSE stream.
...
After the health cutover, the atomic host deployment performs a mandatory loopback MCP initialize and `terminal_list_agents` call through the constrained read-only deployment-smoke principal. The workflow then verifies that the ChatGPT `web-sandbox.oaiusercontent.com` origin can reach the widget stylesheet/reload endpoints while `/mcp` remains origin-strict. If a dedicated external bearer or complete Cloudflare Access service credential is configured, `scripts/mcp-smoke.mjs` additionally performs a public authenticated MCP initialize, requires an online non-read-only agent owned by that principal, opens the persisted terminal surface, starts a real PTY, writes and reads a marker, and connects to the issued terminal SSE capability using the ChatGPT widget origin. Missing external smoke credentials therefore do not block deployment, while a configured full-agent smoke remains a stronger public-path gate. Because Cloudflare Access can reject an unauthenticated public `/mcp` probe before the app sees `Origin`, the public widget check accepts HTTP 401 as an upstream authentication barrier only after the atomic loopback smoke has already verified the app itself rejects the ChatGPT sandbox origin with HTTP 403 `Invalid Origin`. A deployment is therefore not accepted merely because `/health` or `terminal_list_agents` responds.
